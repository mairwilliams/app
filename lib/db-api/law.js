...
exports.all = function all(fn) {
  ...
  var fields = 'id lawId mediaTitle tag participants votes createdAt updatedAt closingAt ' +
    'publishedAt deletedAt status open closed links author authorUrl authorUrl authorPictureUrl ' +
    'party partyUrl backgroundColor fontColor picture scores scoreParticipants';
 ...
 *
 * @param {String} id Law `id`
 * @param {String} citizen author of the score
 * @param {String} value `nothing` or `little` or `somewhat` or `very`
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 *   - 'proposal' single object created or `undefined`
 * @api public
...
exports.score = function score(id, citizen, value, fn) {
  var query = { _id: id, deletedAt: null };

  log('Proceding to score %s at law %s by citizen %s', value, id, citizen.id || citizen);
  Law
  .findOne(query)
  .exec(function (err, law) {
    if (err) {
    ...
    law.score(citizen.id, value, function(err) {
      if (err) {
        log('Found error %s', err);
        return fn(err);
      };

      log('Scored %s at law %s by citizen %s', value, id, citizen.id || citizen);
      fn(null, law);
    });
  });
};
