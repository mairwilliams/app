...
app.get('/law/all', function (req, res) {
  ...
  api.law.all(function(err, lawDocs) {
    ...
    var keys = [
      ...
      'authorPictureUrl party partyUrl backgroundColor fontColor picture',
...
app.get('/law/:id', function (req, res) {
  ...
  api.law.get(req.params.id, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl',
      'authorUrl authorPictureUrl party partyUrl backgroundColor fontColor picture',
      'scores scoreParticipants nothings littles somewhats verys'
    ].join(' ');
...
app.post('/law/:id/score', restrict, function (req, res) {
  log('Request /law/%s/score', req.param('id'));
