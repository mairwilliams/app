...
app.post('/notifications', restrict, function(req, res, nex) {
  ...
  var replies = notifications.replies;
  var citizen = req.user;
  if (replies) {
    log('Adding replies notifications to user %s', citizen.id);
    citizen.notifications.addToSet('replies');
  } else {
    log('Removing replies notifications to user %s', citizen.id);
    citizen.notifications = citizen.notifications.remove('replies');
  }
  citizen.save(function(err) {
    if (err) return res.send(500);
    res.send(200);
